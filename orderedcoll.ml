   ...
   struct
     ...
     let rec insert (x : elt) (t : tree) : tree =
       ...
       | Branch (left, lst, right) -> 
         let comp = C.compare (List.hd lst) x in
         if comp = Equal then Branch (left, x :: lst, right)
         else if comp = Greater then Branch (insert x left, lst, right)
         else Branch (left, lst, insert x right) ;;
     ...
     let rec getmax (t : tree) : elt =
       match pull_min t with
       | lst, Leaf -> List.hd (List.rev lst)
       | _, min_removed -> getmax min_removed
     ...
     let test_insert () =
       ...
       let t = insert w t in
       assert (t = Branch(Branch(Branch (Leaf, [w], Leaf), [z;z], Leaf), [x; x],
       Branch(Leaf, [y], Leaf)));
       ()
     ...
     let test_search () =
       ...
       let t = insert x empty in
       assert (search x t);
       let order = [ false; false; false; false; false; false; false] in
       ...
       let t = insert x empty in
       assert (search x t);
       let order = [ false; false; false; false; false; false; true] in
     ...
     let test_getmax () =
       ...
       assert (getmax (insert x (insert x (insert x empty))) = x);
       (* If you are doubting this case, check the instructions for get_max*)
       assert (getmax (Branch (Leaf, [x ;x2;x3], Leaf)) = x3)
       (* assert (getmax empty = x )  successful calls the exception empty*)
     ...
     let test_getmin () =
       ...
       assert (getmin (insert x (insert x (insert x (insert x empty)))) = x);
       (* We make sure that the function returns that was inserted first / aka the last one in the list *)
       assert (getmin (Branch (Leaf, [x ;x2;x3], Leaf)) = x3)
